(() => {
  ...
  function drawFont5x7(grid, glyph, x0, y0, scale, colorKey) {
    const rows = FONT5X7[glyph];
    if (!rows) return;
    for (let y = 0; y < rows.length; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        ...
        for (let sy = 0; sy < scale; sy++) {
          for (let sx = 0; sx < scale; sx++) setPx(grid, x0 + x * scale + sx, y0 + y * scale + sy, colorKey);
        }
      }
    }
  }

  function buildAsciiGlyph(glyph, colorKey) {
    const g = makeEmptyGrid();
    const scale = 2;
    const gw = 5 * scale;
    const gh = 7 * scale;
    const x0 = Math.floor((GRID - gw) / 2);
    const y0 = Math.floor((GRID - gh) / 2);
    drawFont5x7(g, glyph, x0, y0, scale, colorKey);
    return g;
  }

  function buildAsciiSpecs() {
    const out = [];
    const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
    const numbers = "0123456789".split("");
    const symbols = ["?", "!", "@", "#", "$", "%", "&", "+", "-", "=", "/", "*", "<", ">"];

    for (const ch of letters) out.push(makeSpec(`LETTER ${ch}`, () => buildAsciiGlyph(ch, "y")));
    for (const ch of numbers) out.push(makeSpec(`NUMBER ${ch}`, () => buildAsciiGlyph(ch, "a")));
    for (const ch of symbols) out.push(makeSpec(`SYMBOL ${ch}`, () => buildAsciiGlyph(ch, "m")));
  ...
  const SPECS = [...BASE_SPECS, ...buildAsciiSpecs()];
